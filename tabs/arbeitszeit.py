...
def arbeit_berechn(stundenlohn, ausgabe):
    ...
    if einheit == 'Gemischt':
        zeit = f'\n### {stunden} {s_einheit} und {minuten:.0f} Minuten'

    else:
        zeit = f'{arbeitszeit:.0f} {einheit}'
...
def render(df):
    ...
    if 'stundenlohn' not in st.session_state:
        st.session_state.stundenlohn = 15


    st.markdown("# ⌚️ Invesitierte Arbeit")

    df = df.sort_values(by='Zeitstempel', ascending=False)
    ...
    # Ausgabe und zufallsbutton in eine erweiterunsbox
    with st.expander('⚙️ Einstellungen'):
        gewählter_index, _ = st.selectbox(
            'Ausgabe',
            options,
        ...
        if st.button('Zufällig wählen', width='stretch'):
            st.session_state.ausgabe = df.sample()
            st.rerun()
        # Stundenlohn einstellung
        st.session_state.stundenlohn = st.number_input(
        "Stundenlohn",
        min_value=1,          
        value=15,        
        )
    ...
    arbeitzzeit = arbeit_berechn(stundenlohn, ausgabe)

    # Datum formatieren
    dt = ausgabe['Zeitstempel'].iloc[0] 
    datum = (
        f"{WOCHENTAGE[dt.weekday()]}, "
        f"{dt.day:02d}. {MONATE[dt.month - 1]} {dt.year} "
        f"um {dt.hour:02d}:{dt.minute:02d} Uhr"
        )
    ...
    with col2:
        ...
        st.caption(f'{datum}')
        st.markdown(f'## {ausgabe['Betrag'].iloc[0]} € = {arbeitzzeit}')


    st.markdown("---")


    with st.expander('Reflextion Starten'):
